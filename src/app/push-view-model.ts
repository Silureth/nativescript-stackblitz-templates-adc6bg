...
import * as applicationSettings from '@nativescript/';//tns-core-modules/application-settings';
...
export class PushViewModel extends Observable {
  private static APP_REGISTERED_FOR_NOTIFICATIONS =
  ...
  constructor() {
    ...
    if (
      applicationSettings.getBoolean(
        PushViewModel.APP_REGISTERED_FOR_NOTIFICATIONS,
        false
      )
    ) {
  ...
  public doRegisterForInteractivePush(): void {
    if (!platform.isIOS) {
      ...
      console.log(
        "##### Also, please make sure you don't include the 'click_action' notification property when pusing to Android."
      );
      return;
    }
    ...
    model.iosSettings.interactiveSettings =
      new messaging.IosInteractivePushSettings();
    model.iosSettings.interactiveSettings.actions = [
      ...
      },
      {
        ...
        options:
          messaging.IosInteractiveNotificationActionOptions.foreground |
          messaging.IosInteractiveNotificationActionOptions
    ...
    model.onNotificationActionTakenCallback = (
      ...
      console.log(
        `onNotificationActionTakenCallback fired! \n\r Message: ${JSON.stringify(
          message
